...
from falshfsfse.models import Filieres, Institutions, Regions, DossierUnivs, Nations, Departements, Niveaus, \
    TypeDiplomes, Mentions
...
ECOLE = {
    9 : "CRFD Arts, Langues et Cultures",
    12 : "CRFD Sciences Humaines, Sociales et Educatives",
    11 : "CRFD Sciences et Technologies",
    10 : "CRFD Sciences de la Vie, Santé et Environnement",
}
...
def civilite(request, ecole=9):
    # Construire le formulaire, soit avec les données postées,
    # soit vide si l'utilisateur accède pour la première fois
    # à la page.
    #context = {'civilite_form': CivilForm(), 'filiation_form': FiliationForm()}
    ecole = int(ecole)
    if ecole > 12 or ecole < 9 :
        ecole = 1
    request.session['code_ecole'] = ecole
    request.session['ecole'] = ECOLE[ecole]
    context = {'civilite_form': CivilForm()}
    return render(request, 'ecoledoctorale/civilite.html', context)
...
def filiation(request):
    ...
    return render(request, 'ecoledoctorale/filiation.html', context)

def ecole(request):
    # Construire le formulaire, soit avec les données postées,
    # soit vide si l'utilisateur accède pour la première fois
    # à la page.
    context = {'ecole_form': EcoleForm()}
    return render(request, 'ecoledoctorale/ecole.html', context)
...
def sport(request):
    ...
    return render(request, 'ecoledoctorale/sport.html', context)
...
def autres(request):
    ...
    return render(request, 'ecoledoctorale/autre.html', context)

def search(request):
    # Page qui affiche le champs de recherche pour afficher ses infos
    context = {'search_form': SearchForm()}
    return render(request, 'ecoledoctorale/search.html', context)
...
def fiche(request,code):
    ...
    return render(request, 'ecoledoctorale/fiche.html', context)

""" RECUPERATION ET ENVOIE DES DONNEES EN BD
    -----------------------------------------------------------------------
"""
...
def generate_unique_code(date_of_birth):
    ...
    code = "3" + partial_code_2[:random_index] + add_random[1:3] + partial_code_2[random_index:]
...
def validation_step_civilite(request):
    ...
    if request.method == 'POST':
        try:
            current_registration.matricule_doc_uy1 = request.POST['matricule']
...
def validation_step_ecole(request):
    """
    ...
    if request.method == 'POST':
        try:
            ...
            current_registration.matricule_doc_ext = request.POST['matricule_ext']
            current_registration.ecoledoc = request.POST['ecoledoc']
            current_registration.uniterecherche = request.POST['uniterecherche']
            current_registration.disponibilite = request.POST['disponibilite']
            current_registration.formationinitiale = request.POST['formationinitiale']
            current_registration.naturelicence = request.POST['naturelicence']
            current_registration.optionlicence = request.POST['optionlicence']
            current_registration.anneelicence = request.POST['anneelicence']
            current_registration.naturemasterun = request.POST['naturemasterun']
            current_registration.specialitemasterun = request.POST['specialitemasterun']
            current_registration.etablissementmasterun = request.POST['etablissementmasterun']
            current_registration.departementmasterun = request.POST['departementmasterun']
            current_registration.anneemasterun = request.POST['anneemasterun']
            current_registration.sourcefinancement = request.POST['sourcefinancement']
            current_registration.motivationthese = request.POST['motivationthese']
            current_registration.resumethese = request.POST['resumethese']

            current_registration.cycle = request.POST['cycle']
            current_registration.encadreur1 = request.POST['encadreur1']
            current_registration.encadreur2 = request.POST['encadreur2']
...
class ViewPDF(View):
    def get(self, request, code, *args, **kwargs):
        ...
        pdf = render_to_pdf('ecoledoctorale/pdf_template.html', context)
...
class DownloadPDF(View):
    def get(self, request, code, *args, **kwargs):
        ...
        filename = "Fiche_CRFD_%s.pdf" % (code)
